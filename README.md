...
![add webhook](doc/pic/add-webhook-2.png)

可以点击上面的webhook url进行测试，默认会获取最近一次的delivery，如果webhook没有生效，点击该链接同样可以知晓具体原因。

## 补充说明
### Dockerfile
- 项目根目录下的Dockerfile在镜像内编译Maven + Springboot产生应用镜像。缺点是每次编译都需要下载依赖。
- 如果后面要改为，Jenkins编译Jar包再打包镜像，或是前后端分别产生镜像。就需要对文件进行修改。

### Jenkinsfile
- 主要注意文件中出现的docker凭证和k8s凭证的名称，需要和前面jenkins中创建的一致。
- 此外步骤中的docker仓库地址是写死的，需要根据实际修改，或是参数化。

### k8s yaml 文件
- 一个简单只有无状态应用的例子。如果要包括数据库，需要改写这个文件
- 注意应用镜像名（名称中包括仓库地址）在文件中写死的。
- Ingress host 也是写死的。这个地址由两部分组成。
  `<自定义应用名称>.<ingress地址>` 
  后面部分在ACK集群信息基本信息页面可以找到，测试域名。
